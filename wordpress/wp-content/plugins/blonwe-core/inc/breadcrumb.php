...
if ( ! function_exists( 'blonwe_breadcrubms' ) ) {
    function blonwe_breadcrubms() {
        ...
        $separator  = '';
        $link       = apply_filters( 'blonwe_breadcrumb_link', '<li><a  href="%1$s" title="%2$s" rel="bookmark">%2$s</a> </li>  ' );
        $current    = apply_filters( 'blonwe_breadcrumb_current', '<li><span>%s</span></li>' );
        ...
        if ( $on_front == "page" && is_home() ) {
            $blog_title = isset( $blog_page ) ? get_the_title( $blog_page ) : esc_html__( 'Our Blog', 'blonwe-core' );
            $out .= sprintf( $link, home_url(), esc_html__( 'Home', 'blonwe-core' ) ) . $separator . sprintf( $current, $blog_title );
        } else {
            $out .= sprintf( $link, home_url(), esc_html__( 'Home', 'blonwe-core' ) );
        }
        ...
        if ( is_singular() ) {
            ...
            if ( is_post_type_archive() ) {
                ...
                $out .= $separator . esc_html__( 'Category : ', 'blonwe-core' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_tag() ) {
                $out .= $separator . esc_html__( 'Tag : ', 'blonwe-core' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_date() ) {
                $out .= $separator;
                if ( is_day() ) {
                    global $wp_locale;
                ...
                $out .= $separator . sprintf( $current, esc_html__( 'Error 404', 'blonwe-core' ) );
            } else if ( is_search() ) {
                $out .= $separator . sprintf( $current, esc_html__( 'Search', 'blonwe-core' ) );
            }

        }

        $out .= '';
        return  '<ul>'.apply_filters( 'blonwe_breadcrumbs_out', $out ).'</ul>';
    }
}
// Hooks into the blonwe theme
add_action( 'blonwe_before_template_header', 'blonwe_breadcrubms' );