...
function blonwe_home_slider8($settings){
	...
	if ( $settings['slider_items'] ) {
		...
		foreach ( $settings['slider_items'] as $item ) {	
			...
			echo '<div class="klb-banner for-slider inner-style '.esc_attr($colortype).' align-center justify-start space-60 w-50">';
			...
			echo '<h2 class="entry-title font-md-52">'.esc_html($item['slider_title']).'</h2>';
