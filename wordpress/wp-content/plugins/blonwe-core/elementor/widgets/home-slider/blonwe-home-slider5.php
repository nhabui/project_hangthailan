...
function blonwe_home_slider5($settings){
	...
	if ( $settings['slider_items'] ) {
		...
		foreach ( $settings['slider_items'] as $item ) {
			...
			echo '<div class="klb-banner for-slider inner-style '.esc_attr($colortype).' align-center justify-center space-60 w-90 custom-height" style="height: 770px">';
			...
			echo '<h2 class="entry-title font-md-52 weight-600">'.esc_html($item['slider_title']).'</h2>';
			echo '<div class="entry-excerpt font-md-18 weight-400 not-opacity large">';
			echo '<p>'.esc_html($item['slider_desc']).'</p>';
			echo '</div>';
			echo '</div>';
			echo '<div class="entry-footer">';
			if($item['slider_btn_title']){
				echo '<a href="'.esc_url($item['slider_btn_link']['url']).'" class="btn primary icon-right size-lg" '.esc_attr($target.$nofollow).'>'.esc_html($item['slider_btn_title']).'  <i class="klb-icon-right-arrow-large"></i></a>';
			}
			echo '</div>';
			echo '</div>';
			echo '</div>';
