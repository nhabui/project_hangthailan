...
function blonwe_home_slider3($settings){
	...
	if ( $settings['slider_items'] ) {
		...
		foreach ( $settings['slider_items'] as $item ) {
			...
			$nofollow = $item['slider_btn_link']['nofollow'] ? ' rel="nofollow"' : '';	
				...
				echo '<div class="klb-banner for-slider inner-style '.esc_attr($colortype).' align-center justify-center space-60 w-80">';
				...
				echo '<h2 class="entry-title font-md-60">'.esc_html($item['slider_title']).'</h2>';
				echo '<div class="entry-excerpt font-md-20 weight-300 not-opacity large">';
				echo '<p>'.esc_html($item['slider_desc']).'</p>';
				echo '</div>';
				echo '</div>';
				echo '<div class="entry-footer">';
				if($item['slider_btn_title']){
					echo '<a href="'.esc_url($item['slider_btn_link']['url']).'" '.esc_attr($target.$nofollow).' class="btn primary icon-right">'.esc_html($item['slider_btn_title']).'  <i class="klb-icon-right-arrow-large"></i></a>';
				}
				echo '</div>';
				echo '</div>';
				echo '</div>';
