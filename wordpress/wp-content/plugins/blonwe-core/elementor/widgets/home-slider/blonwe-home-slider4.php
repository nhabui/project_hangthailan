...
function blonwe_home_slider4($settings){
	...
	if ( $settings['slider_items'] ) {
		...
		foreach ( $settings['slider_items'] as $item ) {
			...
			echo '<div class="klb-banner for-slider inner-style '.esc_attr($colortype).' align-center justify-center space-60 w-90 custom-height" style="height: 860px">';
			...
			if($item['slider_btn_title']){
				echo '<a href="'.esc_url($item['slider_btn_link']['url']).'" class="btn default outline icon-right" '.esc_attr($target.$nofollow).'>'.esc_html($item['slider_btn_title']).'  <i class="klb-icon-right-arrow-large"></i></a>';
			}
			echo '</div>';
			echo '</div>';
			echo '</div>';
