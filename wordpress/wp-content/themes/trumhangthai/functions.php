...
// Add custom fields to a WooCommerce product metabox
function nhabt_wpwoo_add_custom_fields_metabox() {
    add_meta_box(
        'nhabt_wpwoo_custom_fields',
        __('Custom Product Fields', 'nhabt-wpwoo'),
        'nhabt_wpwoo_render_custom_fields_metabox',
        'product',
        'normal',
        'high'
    );
...
add_action('add_meta_boxes', 'nhabt_wpwoo_add_custom_fields_metabox');

// Callback function to render the content of the metabox
function nhabt_wpwoo_render_custom_fields_metabox($post) {
    // Text Field
    woocommerce_wp_text_input(array(
        'id'          => '_custom_text_field',
        'label'       => __('Link Facebook', 'nhabt-wpwoo'),
        'placeholder' => __('Enter Link', 'nhabt-wpwoo'),
        'desc_tip'    => true,
        'description' => __('This is a custom text field.', 'nhabt-wpwoo'),
        'value'       => get_post_meta($post->ID, '_custom_text_field', true),
    ));
}

// Save custom fields when the product is saved
function nhabt_wpwoo_save_custom_fields($post_id) {
    // Save Text Field
    $custom_text_field = sanitize_text_field($_POST['_custom_text_field']);
    update_post_meta($post_id, '_custom_text_field', $custom_text_field);
}

add_action('woocommerce_process_product_meta', 'nhabt_wpwoo_save_custom_fields');
